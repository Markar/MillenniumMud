...
class CmdMorph(Command):
    ...
    def func(self):
        ...
        try:
            ...
            caller.traits.ep.current -= my_glvl
            caller.traits.hp.current += my_glvl
...
class CmdEnergyControl(Command):
    ...
    def func(self):
        ...
        if caller.traits.fp.current < self.fp_cost:
            caller.msg(f"|rYou can't focus well enough to use this power.")
            return

        if caller.traits.ep.current < self.ep_cost:
        ...
        if active:
            ...
            caller.traits.ep.current -= self.ep_cost
            return
...
class CmdEngulf(Command):
    ...
    def func(self):
        ...
        glvl = caller.db.guild_level
        hp = caller.traits.hp.current
        hpmax = caller.traits.hp.base
        fp = caller.traits.fp.current
        fpmax = caller.traits.fp.base
        ...
        if caller.traits.ep.current < self.cost:
            self.msg(f"|rYou don't have enough energy to use this power.")
            return

        power = math.ceil(caller.traits.hp.base * (9 + caller.db.guild_level / 7) / 10)
        fpower = math.ceil(caller.traits.fp.base * (9 + caller.db.guild_level / 7) / 10)
        caller.traits.hp.current += power
        caller.traits.fp.current = fpower
        caller.traits.ep.current -= self.cost
        caller.db.engulfs -= 1
...
class CmdCellularReconstruction(Command):
    ...
    def func(self):
        ...
        if gl < self.guild_level:
            ...
            return
        if caller.traits.ep.current < self.epcost:
            self.msg(f"|rYou don't have enough energy to use this power.")
            return
        if caller.traits.fp.current < self.fpcost:
            self.msg(f"|rYou can't focus well enough to use this power.")
            return
        if not caller.cooldowns.ready("reconstruction"):
            self.msg(f"|rYou can't use this power yet.")
            return
        if caller.traits.hp.current == caller.traits.hp.base:
        ...
        caller.traits.hp.current += to_heal
        caller.traits.ep.current -= self.epcost
        caller.traits.fp.current -= self.fpcost  # not working with overmax?
        caller.msg(
