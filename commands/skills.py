...
class CmdStatSheet(Command):
    ...
    def func(self):
        ...
        table.add_row(
            f"|GHp",
            f"|C {int(caller.traits.hp.current) or 0} / {int(caller.traits.hp.base)}",
        ),
        table.add_row(
            f"|GFp",
            f"|C {int(caller.traits.fp.current) or 0} / {int(caller.traits.fp.base)}",
        ),
        table.add_row(
            f"|GEp",
            f"|C {int(caller.traits.ep.current) or 0} / {int(caller.traits.ep.base)}",
        ),
...
class CmdAdvance(Command):
    ...
    def adv_player_level(self):
        ...
        caller.db.weight.base = 50 + caller.db.level * 2 + caller.db.str * 3
