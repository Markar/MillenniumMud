...
class CmdAdv50(Command):
    ...
    def func(self):
        ...
        self.caller.traits.hp.current = 1000
        self.caller.traits.fp.current = 1000
        self.caller.traits.ep.current = 1000
        self.caller.traits.hp.base = 1000
        self.caller.traits.fp.base = 1000
        self.caller.traits.ep.base = 1000
...
class RespawnMobs(Command):
    ...
    def func(self):
        ...
        for room in MobRoom.objects.all_family():
            caller.msg(f"|gRespawning mobs in {room}.")
            room.respawn_mobs()
...
class AddToGauge(Command):
    key = "addgauge"

    def func(self):
        caller = self.caller
        caller.traits.add("test", "Test", trait_type="gauge", base=100, mod=0, rate=0.1)
        caller.traits.test.mod = 200
        caller.msg(
            f"|gGauge: base {caller.traits.test.current} value {caller.traits.test.current}/{caller.traits.test.base}."
        )
...
class Setup(Command):
    ...
    def func(self):
        caller = self.caller
        # caller.msg(f"Setting up for {caller} and trait {caller.traits.hp.current} hp.")
        # tickerhandler.clear()
        # caller.msg(f"all tickers: {tickerhandler.all()}")
        # tickerhandler.remove
        caller.traits.add("hp", "Health", trait_type="gauge", base=402, mod=0, rate=0.1)
        caller.traits.add("fp", "Focus", trait_type="gauge", base=77, mod=0, rate=0.1)
        caller.traits.add("ep", "Energy", trait_type="gauge", base=900, mod=0, rate=0.1)
        caller.traits.add("speed", "Speed", trait_type="static", base=10, mod=0)
        del caller.db.hp
        del caller.db.hpmax
        del caller.db.fp
        del caller.db.fpmax
        ...
        del caller.db.epmax
        del caller.db.speed
        caller.msg(f"added traits")


class UpdateEverything(Command):
    key = "updateeverything"

    def func(self):
        caller = self.caller
        for npc in Character.objects.all_family():
            caller.msg(f"Updating {npc}")

            npc.traits.add(
                "hp", "Health", trait_type="gauge", base=402, mod=0, rate=0.1
            )
            npc.traits.add("fp", "Focus", trait_type="gauge", base=77, mod=0, rate=0.1)
            npc.traits.add(
                "ep", "Energy", trait_type="gauge", base=900, mod=0, rate=0.1
            )
            npc.traits.add("speed", "Speed", trait_type="static", base=10, mod=0)
            del npc.db.hp
            del npc.db.hpmax
            del npc.db.fp
            del npc.db.fpmax
...
class DeleteAllNpcs(Command):
    key = "deleteallnpcs"

    def func(self):
        caller = self.caller
        for npc in NPC.objects.all_family():
            caller.msg(f"Deleting {npc}")
            npc.delete()
            # npc.delete()
...
class GeneralCmdSet(CmdSet):
    ...
    def at_cmdset_creation(self):
        ...
        self.add(JoinPlayer)
        self.add(Setup)
        self.add(UpdateEverything)
        self.add(DeleteAllNpcs)
