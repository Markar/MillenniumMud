...
class CmdPulseGrenade(PowerCommand):
    ...
    def func(self):
        ...
        for obj in caller.location.contents:
            if obj.traits.hp.current:
                obj.traits.hp.current -= 10 + skill_rank * 2
                # obj.db.stunned = True
...
class CmdFirstAid(PowerCommand):
    ...
    def func(self):
        ...
        caller.traits.hp.current += amount
        ...
        #     if obj.traits.hp.current:
        #         obj.traits.hp.current += 10 + skill_rank * 2
        #         if obj.traits.hp.current > obj.db.hpmax:
        #             obj.traits.hp.current = obj.db.hpmax
...
def _calculate_resupply_restoration(self):
    ...
    caller.traits.fp.current += fp_restored
    caller.traits.ep.current += ep_restored
    caller.msg(caller.get_display_status(caller))
...
class CmdResupply(PowerCommand):
    ...
    def func(self):
        ...
        if caller.traits.ep.current == caller.traits.ep.base:
...
class ResupplyBuff(BaseBuff):
    ...
    def at_tick(self, initial, **kwargs):
        ...
        if (
            ...
            and self.owner.traits.fp.current == self.owner.db.fpmax
        ):
            self.duration = 0
...
class CmdGAdvance(Command):
    ...
    def _adv_level(self):
        ...
        if caller.db.gxp < cost:
            ...
            caller.traits.ep.base = 50 * caller.db.guild_level + ep_max_bonus
