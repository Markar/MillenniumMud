...
class CmdAdaptiveArmor(PowerCommand):
    ...
    def func(self):
        ...
        if not caller.cooldowns.ready("adaptive_armor"):
            ...
            return False
        if caller.traits.ep.current < self.cost and not caller.db.adaptive_armor:
...
class CmdDocWagon(PowerCommand):
    ...
    def func(self):
        ...
        caller.traits.hp.current = caller.traits.hp.base
        caller.traits.fp.current = caller.traits.fp.base
...
class CmdAdrenalineBoost(PowerCommand):
    ...
    def func(self):
        ...
        if caller.traits.ep.current < self.cost and not adrenaline:
            caller.msg(f"|rYou need more energy to do that.")
            return
