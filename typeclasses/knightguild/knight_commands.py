...
class CmdBindWounds(PowerCommand):
    ...
    def func(self):
        ...
        if glvl < self.guild_level:
            ...
            return
        if hppercent := caller.traits.hp.current / caller.traits.hp.current * 100:
            if hppercent > 45:
    ...
    def _end_bind_wounds(self, caller):
        ...
        self.traits.hp.current += 10 + caller.db.glvl + caller.db.skills["bind wounds"]
...
class CmdMeditate(Command):
    ...
    def _end_meditation(self, caller):
        ...
        caller.traits.hp.current += hp_restored
