...
class Changelings(PlayerCharacter):
    ...
    def at_tick(self):
        ...
        if self.db.regrowth and hp < hpmax:
            ...
            if ep >= regrowth_cost and hp < hpmax:
                ...
                self.traits.ep.current -= regrowth_cost
            else:
        ...
        self.traits.hp.current += hp_regen_amt
        self.traits.fp.current += base_fp_regen
        self.traits.ep.current += base_ep_regen + ep_bonus
    ...
    def get_display_status(self, looker, **kwargs):
        ...
        if looker != self:
            ...
            if self.key == "Markar":
                chunks.append(f"{looker.traits.hp.current} / {looker.traits.hp.base}")
            # append mob's health status ({looker.traits.hp.current})
