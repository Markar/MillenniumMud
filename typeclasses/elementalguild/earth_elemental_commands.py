...
class CmdMountainStance(PowerCommand):
    ...
    def func(self):
        ...
        caller.traits.hp.base = (
            50 + caller.traits.con.value * caller.db.con_increase_amount
        )
...
class CmdEarthShield(PowerCommand):
    ...
    def func(self):
        ...
        if glvl < self.guild_level:
            caller.msg(
                f"|CYou need to be guild level {self.guild_level} to use earth shield."
            )
            return
...
class CmdMudPatch(PowerCommand):
    ...
    def func(self):
        ...
        amt = 20 + caller.db.skills.get("elemental harmony", 1) * 2
        caller.traits.hp.current += amt
        caller.traits.ep.current -= self.cost
        caller.cooldowns.add("global_cooldown", 2)
...
class CmdEarthenRenewal(PowerCommand):
    ...
    def func(self):
        ...
        if glvl < self.guild_level:
            self.msg(f"|rYou must be at least {self.guild_level} to use this power.")
            return
        if not caller.traits.ep.current or caller.traits.ep.current <= energy_cost:
        ...
        caller.traits.ep.current -= energy_cost
...
class CmdTremor(PowerCommand):
    ...
    def func(self):
        ...
        if glvl < self.guild_level:
            self.msg(f"|rYou must be at least {self.guild_level} to use this power.")
            return
        if caller.traits.fp.current < self.cost:
...
class CmdPowers(Command):
    ...
    def func(self):
        ...
        table.add_row(f"|GMud Patch", 2, "Healing", "10 Energy")
