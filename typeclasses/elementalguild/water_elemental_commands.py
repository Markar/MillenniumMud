    ...
    def func(self):
        if not self.args:
            ...
            if corpse := caller.location.search("corpse-1"):
                ep = caller.traits.ep.current
                epmax = caller.traits.ep.base
                power = corpse.db.power

                if ep + power > epmax:
                    caller.traits.ep.current = epmax
                else:
                    caller.traits.ep.current += max(power, 0)
                corpse.delete()
...
class CmdWaterJet(PowerCommand):
    ...
    def func(self):
        ...
        if not caller.traits.fp.current >= self.cost:
...
class CmdRejuvenate(PowerCommand):
    ...
    def func(self):
        ...
        if caller.traits.ep.current < cost:
            self.msg(f"|rYou can't seem to focus on restoring your form.")
            return

        if hp + to_heal > hpmax:
            self.traits.hp.current += hpmax
        else:
            self.traits.hp.current += max(to_heal, 0)

        if fp + to_heal > fpmax:
            caller.trait.fp.current = fpmax
        else:
            caller.trait.fp.current += max(to_heal, 0)

        caller.trait.ep.current -= cost
