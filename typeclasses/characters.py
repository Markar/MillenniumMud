...
class Character(ObjectParent, ClothedCharacter):
    ...
    def can_flee(self):
        ...
        if (randint(0, 99) - self.traits.fp.current) < evade:
            return True
        else:
    ...
    def at_object_creation(self):
        ...
        self.traits.add("speed", "Speed", trait_type="static", base=10, mod=0)

        self.traits.add("hp", "Health", trait_type="gauge", base=50, mod=0, rate=0.1)
    ...
    def use_skill(self, skill_name, *args, **kwargs):
        ...
        return skill_trait.current + stat_bonus
    ...
    def get_display_status(self, looker, **kwargs):
        ...
        fp = self.traits.fp.current
        fpmax = self.traits.fp.base
        ep = self.traits.ep.current
        epmax = self.traits.ep.base
        chunks.append(
    ...
    def revive(self, reviver, **kwargs):
        ...
        if self.tags.has("unconscious"):
            ...
            self.traits.hp.current = self.traits.hp.base // 5
...
class PlayerCharacter(Character):
    ...
    def use_heal(self):
        ...
        if hp + damage > hpmax:
            ...
            hp_amount = hpmax - hp
            # self.traits.hp.current += max(damage, 0)
            self.msg(f"in use_heal {self.traits.hp.current}")
            self.traits.hp.current += 100

        if ep + damage > epmax:
            ep_amount = epmax - ep
            self.traits.ep.current = epmax
        else:
            ep_amount = epmax - ep
    ...
    def at_pc_tick(self):
        ...
        self.traits.fp.current += base_fp_regen
...
class NPCHeal(BaseBuff):
    ...
    def at_tick(self, initial, **kwargs):
        ...
        owner.traits.hp.current += to_heal
...
class NPC(Character):
    ...
    def at_pre_attack(self, wielder, **kwargs):
        ...
        if self.traits.ep.current < weapon.get("energy_cost", 5):
            return False
